...
- 全量 `Library` 导出必须使用 SQLite 源索引作为依赖底座。默认命令应自动查找输出目录或输入目录里的 `unity_source_index.db`；找不到时自动在输出目录构建一份。`--source_index` 用于显式指定共享源索引。旧 CAB map / AssetMap 只作为显式 `--map_op` 调试或兼容旧流程，不能作为精品全量导出的默认路径。
