...
3. 自动生成绑定索引。
   `asset_catalog.jsonl`、`animation_bindings.jsonl` 和后续增强的 `model_animations.json` 负责说明“哪些动画可能适配哪些模型”。
4. 按需生成预览 glTF。
...
- 绑定关系优先通过同批依赖、Animator Controller、AnimationClip container、骨骼路径、skeleton hash、bone path 命中率建立。
- 只有显式 `--animation_package Embedded` 或 `Both`，或后续专门的预览/打包命令，才把动画写进模型 glTF。
- `animation_bindings.jsonl` 目前是候选索引，不等于最终可播放验证结果；后续会补“实际写入 glTF 后 channel 数”的验证结果。
