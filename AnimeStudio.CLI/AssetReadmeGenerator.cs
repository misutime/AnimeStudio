...
namespace AnimeStudio.CLI
{
    internal static class AssetReadmeGenerator
    {
        public static void Generate(string savePath)
        {
            ...
            {
                var output = ResolveLibraryOutput(savePath, (string)model["output"]);
        ...
        private static void WriteLibraryReadme(string savePath, int modelCount, JObject compact, JObject assemblies)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("1. `Models/.../ASSET_README.md`：单个模型怎么用，材质、骨骼、模块有什么注意事项。");
            sb.AppendLine("2. `Models/.../MATERIAL_REPORT.md`：单个模型的材质/贴图/tint 细节。");
            sb.AppendLine("3. `character_assemblies.json`：body、face、hair、accessory 等模块组装候选。");
            sb.AppendLine("4. `animation_out_of_scope.json`：说明本次默认跳过动画资源。");
            ...
            sb.AppendLine("| --- | --- | --- |");
            sb.AppendLine("| `ASSET_README.md` | 人 | 单模型使用入口，汇总基础信息、材质状态、骨骼/skin、模块组装和建议。 |");
            ...
            sb.AppendLine("| `animation_out_of_scope.json` | 人/工具 | 默认跳过动画资源的边界说明和残留动画 catalog 计数。 |");
            ...
            sb.AppendLine("## 设计原则");
            sb.AppendLine();
            sb.AppendLine("- 默认主线只导出模型、贴图、材质、骨骼、索引和报告。");
            sb.AppendLine("- 动画资源默认只识别边界、计数并跳过，不生成正式动画资产或模型-动画候选。");
        ...
        private static void AppendLibraryAnimationStatus(StringBuilder sb, JObject compact)
        {
            ...
            sb.AppendLine("- 需要研究动画时只能使用显式诊断命令；诊断结果不能进入默认素材库能力声明。");
            sb.AppendLine();
        }
            ...
            Dictionary<string, JObject> assemblyByOutput)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("这份说明面向人工使用素材库。机器可读的完整关系仍以 `asset_catalog.jsonl`、`animation_out_of_scope.json`、`character_assemblies.json`、glTF `extras` 和各类 validation/report JSON 为准。");
            sb.AppendLine();
            ...
            Dictionary<string, JArray> animationRefsByModel)
        {
                ...
                || candidates.Count == 0)
            {
                ...
                sb.AppendLine("- 当前项目默认不导出、不转换、不验证动画；这不影响模型、贴图、材质和骨骼作为素材使用。");
                sb.AppendLine();
                return;
            }
            ...
            foreach (var candidate in candidates.OfType<JObject>().Take(12))
            {
                ...
                sb.AppendLine($"- `{name}`（显式诊断/旧库迁移线索）");
            ...
            {
                sb.AppendLine($"- 其余 `{candidates.Count - 12}` 条旧诊断候选请查看显式动画诊断索引；它们不属于默认 Library 输出。");
            }
            sb.AppendLine();
        }
        ...
        private static void AppendUseAdvice(StringBuilder sb, JObject model, string output)
        {
            sb.AppendLine("## 使用建议");
            sb.AppendLine();
            sb.AppendLine("- 默认优先打开 glTF/GLB 查看模型、材质和骨骼。");
            ...
            {
                sb.AppendLine("- 这是带骨骼模型；骨骼/skin 属于模型数据，应尽量保留，不代表默认会导出动画。");
            }
            sb.AppendLine();
        }
        ...
        private static string ResolveLibraryOutput(string savePath, string output)
        {
