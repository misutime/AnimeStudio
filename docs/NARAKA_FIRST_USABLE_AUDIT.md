...
smoke 会额外检查 `material_sidecars` 中的自定义 shader 行是否保留 `key_texture_slots_json`、`exported_textures_json`、`unresolved_steps_json` 和 `raw_json`。`shader_name` 目前在既有 Face 样本中仍为空，smoke 会记录 `missingShaderNameRows` 作为后续上游材质 sidecar 追踪项，但不把少量私有 shader 名缺失误判成贴图丢失或材质错绑。
