...
- `smoke_summary.json`：机器读 smoke 摘要，用来复查产物路径、`assetLibraryContract`、`modelReportEntrypoints`、能力标记、验证状态、SQLite 索引计数、`qualityGates`、`hadiModularBoundary`、`shaderBoundary`、`staticEnvironment`、`characterCandidate`、`characterCandidateSourceIndexBoundary`、`sourceIndexLegacyAnimationClipDomains`、`animatorControllerProductionGate`、`monoBehaviourAnimationClipPPtrSummary`、`scriptAnimationComponentDiagnostics` 和动画诊断状态。关键动画边界诊断同时保留在顶层和 `animationRelationCoverage` 下，方便机器审计直接读取。脚本写完后会立刻反读解析，避免报告可读但机器摘要损坏。
