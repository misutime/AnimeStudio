...
2026-06-26 继续复验带 glTF validator 的 quick smoke `D:\Assets\Naraka\Naraka_FirstUsableSmoke_VerifiedPreview_GltfValidation_Quick_Current`（`-SkipBrowserValidation -SkipAnimationDiagnostic`）通过。最终 `smoke_summary.json` 写出 `zhumuScriptAnimationProbe.gltfValidation=ok`、`verifiedPreviewGltfValidation=ok`、`capabilitiesAnimations=true`、`animationAssetRows=1`、`verifiedPreviewAssetRows=1`、`modelAnimationCandidateRows=1`、`modelAnimationRelationRows=1`、`relationAnimationRows=1`、`usableRelationAnimationRows=1`、`productionReadiness=productionPreviewReady`、`previewOnly=true`、`embeddedModelRequired=true`；顶层代表库仍为 `capabilities.animations=false`。`SMOKE_REPORT.md` 同步记录 `Verified preview boundary`、`Verified preview coverage`、SQLite 关系计数和 compact 计数，说明整合 smoke 自身已经能带 validator 验收 Zhumu verified preview，不再只依赖单独手工 validator。
