...
- `smoke_summary.json`：机器读 smoke 摘要，用来复查产物路径、能力标记、验证状态、SQLite 索引计数、`qualityGates`、`shaderBoundary` 和动画诊断状态。脚本写完后会立刻反读解析，避免报告可读但机器摘要损坏。
