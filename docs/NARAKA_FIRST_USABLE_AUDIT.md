...
| Zhumu soul attack prefab | `D:\Assets\Naraka\Naraka_ZhumuSoul_AttackPrefab_ModelProbe_Current` | 脚本动画边界模型样本 | `mo_pve_b_zhumu_soul_01` attack prefab 模型导出通过，`model_validation=ok`、带 skin 和贴图；同名源模型有 `SimpleAnimation` 和 Avatar 结构重叠诊断线索，但默认动画候选仍为 0。 |
...
zhumu_soul_attack_prefab: models=1 textures=10 material_sidecars=2 texture_links=11 link_errors=0 custom_shader_materials=0 validation=ok:1 animation_candidates=0
```
