...
需要临时观察 Renderer-bones 局部 skin 是否能保持 Naraka face 这类自定义网格静态形状时，可以追加 `--naraka_avatar_mesh_renderer_bones_skin_diagnostic`。这个开关只在默认关闭的诊断路径里，为每个 part 单独写 glTF skin：joint 顺序来自 `SkinnedMeshRenderer.m_Bones`，权重来自 `AvatarMeshDataAsset.m_AnimSkinData`，inverse bind matrix 来自转置后的 `m_BindPoses`。为了避免使用尚未证明匹配的 Unity Transform 层级导致静态拉飞，诊断 joint rest 矩阵先由 `inverse(m_BindPoses)` 反推；报告会写 `rendererBonesDiagnosticSkins[*].restSelfCheck`，用 `jointRestMatrix * inverseBindMatrix ~= identity` 记录静态 rest 是否自洽；同时写 `transformHierarchyMatrixCheck`，按 Renderer bone 槽位比较导出的 Transform 逆世界矩阵和 `m_BindPoses` 是否接近，用来解释为什么当前不能直接使用 Unity Transform 层级当 rest。验证和动画门禁仍保持 warning/blocked，例如 `renderer_bones_diagnostic_skin_present`、`renderer_bones_transposed_bind_pose_inverse_matrices_diagnostic`。它只能证明局部权重和 bind pose 槽位自洽，不能作为完整角色 skin、正式装配或动画 smoke 通过证据。
