...
- `--model_source PrefabPrimary`：模型默认以 prefab/Animator 组合体为主，同时保留有明确 Unity 关系或静态素材语义的独立 Mesh；raw fbx/source parts 只进入索引或 `RawUnreferenced`。
- `--texture_mode Png`：贴图默认 PNG，模型目录下有 `Textures` 硬链接，方便直接查看。
- `--animation_package Separate`：动画默认独立入库，不嵌入每个模型。
- 默认 Library 是“完整可浏览素材库”，只过滤非常确定的垃圾或损坏对象。不确定的 debug box、VFX mesh、helper、decal、terrain tile、building part 等优先导出、分类、打标签并写报告；精品筛选后续通过 Core/Curated/Playable 等后处理能力完成。
- 默认 Library 会生成 `VFX/` 特效元数据目录：Unity `ParticleSystem`、`LineRenderer`、`TrailRenderer`、`VisualEffect`、GPU Particle/VFX 对象会进入 `vfx.json` / `VFX_REPORT.md`；mesh 型特效会在 `Models` 中标注 `resourceKind=VFX`。当前这是 metadata-first 管线，记录组件、来源、ParticleSystem/Renderer 预览参数、材质/贴图/Mesh 引用，不代表 ParticleSystem/shader 动画已经完整烘焙为可播放标准格式。Browser 会基于 Unity 元数据、名称、分类、组件、材质、贴图和 Mesh 引用生成 approximate VFX 缩略图/预览，帮助筛选效果类型；它不是 Unity runtime 100% 还原。旧素材库如果缺少 `textureRefCount` / `textures` / 完整 `previewHints`，需要重建 source index 并重新生成 VFX catalog，推荐重新导出一次完整 Library 后再评估特效质量。
...
- `Models` 默认放 Unity prefab、Animator、完整 GameObject 组合模型，以及有明确 Unity 关系或静态素材语义的独立 Mesh，优先保证“素材尽量完整可浏览”。
...
静态环境、建筑、地形块、植被、道具等独立 Mesh 是默认 Library 的一等资源。当前 `StaticMeshPrimary` 的信号包括：
