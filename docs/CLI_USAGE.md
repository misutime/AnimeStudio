...
报告和 catalog 会额外写 `selectedVisualCellTransformNodeRequiredCount`、`selectedVisualCellTransformNodeMissingCount`、`selectedVisualCellTransformNodeCoverageStatus` 和 `sourceSkinMappingStatus`。这些字段只说明选中 `ActorBodyVisualCell.transformNodes.data` 是否足够覆盖 `AvatarBoneWeights` 的最大 boneIndex，以及当前为什么仍不能写 glTF skin。同一个 SerializedFile 里其它 VisualCell 的节点表即使覆盖范围，也只能作为诊断线索；没有确定的 joint 名称/路径映射前，不能拿来绑定选中部件。
