...
CAB map 是完整 Unity 外部引用索引，不是单次导出的裁剪缓存。模型、材质、贴图等 3D 导出会用完整输入目录建立 CAB / SerializedFile / PPtr 依赖图，然后再用 `--containers`、`--names` 过滤导出候选。这样即使只导出一个角色，也不会切断脸、附件、材质、Mesh 等外部 CAB 引用。
...
- `--mode Library`：导出素材库。
- `--group_assets ByLibrary`：按 `Models`、`Textures`、`Materials` 等库目录组织。
...
- `--animation_package Skip`：动画默认不导出、不转换、不验证，只在源索引或报告里作为 out-of-scope 线索记录。
- 默认 Library 先专注 prefab/Animator 模型、贴图、材质、骨骼/skin、索引和报告。普通 Mesh、VFX 和动画属于显式扩展/诊断域，避免默认批量导出被场景碎片、特效网格、VFX metadata 和动作资源放大。
...
> 默认导出结果应该像完整可浏览模型素材库，而不是像 Unity 运行时对象转储，也不是最终精品包。模型要尽量完整覆盖、分类清晰、可追溯；贴图要默认可见；manifest 和报告负责记录源路径、材质状态、可信度和后续修复线索。动画不属于默认导出目标。

更底层的实现准则：

> 骨骼、模型、材质、贴图的绑定关系必须来自 Unity 自身序列化引用和结构。不要把单个游戏的目录习惯、角色命名、资源前缀当成核心逻辑；如无不得已的明确理由，默认逻辑绝不自行推断关系。动画关系默认只作为诊断线索，不生成正式素材库绑定。
...
3. 实际导出验证：glTF node、skin/joint、材质、bbox。
4. 显式 fallback：container、目录名、资源名、游戏 profile。fallback 必须明确标注，不能进入默认绑定结果。

跨游戏验证准则：

- 默认规则必须在多个 Unity 游戏上交叉验证，不能为了某个游戏的目录或命名做核心逻辑特判。
- Freedunk 用于验证篮球人物、球场、道具、模型/贴图/骨骼/skin；VRising、Valheim 等其他 PC Unity 游戏用于验证通用资源入口、bundle/SerializedFile 发现、模型/贴图/骨骼索引规则。
- 跨游戏初期可以接受有少量重复或 debug 基础体混入，但不能系统性漏掉成熟游戏里的建筑、植被、POI、岩石、地形块、道具或静态 Mesh。动画资源不参与默认推荐关系和成功率统计。
...
- 定向导出选中 prefab/container 根对象时，CLI 可能会临时补导同一 Unity container 下的 Renderer owner，避免 prefab 根本身没有直接 Mesh 导致完全看不到模型。这类输出会标记为 `libraryRole=PrefabRendererPart`、`diagnosticOnly=true`、`sourcePartRole=CoveredByPrefab`，只作为材质、Mesh 和后续 prefab 组装诊断线索；它不能计入默认 prefab/Animator 主模型视觉验收，也不应作为素材库推荐主入口。
- 如果同一次定向导出还开启了 `--include_static_meshes`，依赖扩展报告中已经证明属于该 prefab/container Renderer 的 Mesh 会降级为 `StaticMeshSource` + `sourcePartRole=CoveredByPrefab`，只进索引诊断，不再重复升级为 `StaticMeshPrimary`。这样可以避免一个 prefab 组合模型旁边再出现一堆零散重复 mesh；真正的主线目标仍然是导出组合后的 prefab/glTF。
- 导出后如果存在这类 prefab 子件，根目录会写 `prefab_assemblies.json`，按 Unity prefab/container 聚合部件、材质状态和输出路径。它是组装关系报告，不代表主模型已经正式通过；如果同一 container 已经有非诊断的 `PrefabPrimary` / `ModularCharacterBase` 主模型，报告会标记 `existingMainModelPresent=true` 并把子件继续保留为诊断证据，不再重复合成 assembly；如果没有原始主模型，只有写出 `PrefabAssemblyPrimary` 主模型并写入 catalog/SQLite 后，才能计入默认主模型验收。
- `PrefabAssemblyPrimary` 使用短目录和可读文件名，例如 `Models/PrefabAssemblies/brutePrisonerViewBase_ab12cd34/brutePrisonerViewBase_ab12cd34.gltf`。完整 Unity container 路径保留在 `asset_catalog.jsonl`、SQLite、`prefab_assemblies.json` 和说明文件里，避免 F3D/Blender/Windows 工具因为超长路径打不开，同时不让所有模型在 DCC/查看器里都显示成 `model.gltf`。
- 已经被 prefab 覆盖的 `PrefabRendererPart` 文件不留在正式 `Models/` 浏览目录，也不默认保留诊断 glTF。导出器会在生成 `PrefabAssemblyPrimary` / `PrefabAssemblyPartial` / `PrefabAssemblyMissing` 报告后删除这些 `GameObject_n...` 中间文件，并在 catalog/SQLite 继续标记 `diagnosticOnly=true`、`sourcePartRole=CoveredByPrefab`、`diagnosticFileStatus=deletedCoveredByPrefabPart`。它们是组装来源证据，不是最终素材入口。
- 如果 prefab 子件已经导出但组装主模型没有写出，CLI 必须写 `PrefabAssemblyMissing` 诊断行，并标记 `diagnosticOnly=true`、`visualAcceptanceEligible=false`、`visualAcceptanceBlocker=missing_prefab_main_model_or_assembly`。这表示当前只拿到了来源证据，不能用跳过视觉验收的零散 part 假装 prefab 主模型完整。
- 如果能写出合成 glTF，但源索引显示本次导出的、且有可解析 `SkinnedMeshRenderer.mesh` 或 `MeshFilter.mesh` 的 Renderer owner 覆盖率明显偏低，CLI 应写 `PrefabAssemblyPartial`，并带 `expectedRendererOwnerCount`、`exportedRendererOwnerCount`、`rendererOwnerCoverageRatio`、`assemblyCompletenessStatus=partial_source_renderer_coverage`。没有确定 mesh 关系的 Renderer helper 不进入主模型完整性分母，避免把不可导出的 helper 当缺件；材质缺失仍通过 `materialStatus`、`materialMissingRendererBinding`、`MATERIAL_REPORT.md` 和验证报告暴露。这类文件可以打开诊断，但 `visualAcceptanceEligible=false`，不能算默认主模型通过。`modelValidationStatus` 保留主模型完整性结论，glTF 文件结构校验结果另写 `gltfValidationStatus`。
- `assetBundle.containerPreload` 只能证明对象被同一 container 预加载，不能单独证明它属于当前 prefab 层级。定向 prefab assembly 若发现 renderer part 来自其他 prefab container，会跳过合入主 glTF，保留 `assemblySkippedParts.reason=unverified_external_prefab_renderer_part`，并把主模型视觉门禁标成 `visualAcceptanceBlocker=unverified_external_prefab_renderer_parts`，直到有更强的 Unity ownership 证据。
...
SQLite `assets` 表会同步保存 `source_part_role`、`covered_by_prefab_container`、`visual_acceptance_eligible`、`visual_acceptance_scope` 和 `visual_acceptance_blocker`。浏览器、验收脚本和人工抽样应优先用这些字段区分默认主模型、显式静态 Mesh、prefab 子件诊断和主模型缺失诊断。
...
如果 base color / baseColorTexture、UV 和 Renderer 材质关系都正常，静态截图也可识别，就可以把它当作可用的基础材质预览；但这不等于游戏私有 shader 已完整复刻。遇到分层材质、mask、运行时 tint 或顶点色参数时，CLI 会把材质标成 `needsCustomShaderLayer` / `bestEffortDegradedPreview`，并在 `MATERIAL_REPORT.md`、材质 JSON、glTF `extras`、`asset_catalog.jsonl` 和 SQLite 中保留 shader 重建线索。若这些自定义 shader 材质里的顶点色 RGB 会被通用 glTF 查看器误当最终颜色，导出器会抑制对应 primitive 的 `COLOR_0` 预览输出，并在 mesh extras 里记录原因。
...
每个模型目录还会生成 `ASSET_README.md`。这是模型使用入口，会汇总基础信息、材质状态、骨骼/skin、模块/头发/头部/附件组装关系和使用建议。正常浏览素材时先看 `ASSET_README.md`，需要材质细节时再看 `MATERIAL_REPORT.md`。
...
短音效属于开发素材，例如脚步声、命中、技能、按钮、篮球入网、环境点缀等。但它不属于默认 3D Library，不能和模型、贴图、骨骼混在同一个浏览目录里。
...
- `qualityGates.shaderReferenceSidecars` / `shaderReferenceOnlySidecars` / `customShaderReferenceSidecars`：材质 sidecar 中已结构化保留 Unity `Material.m_Shader` PPtr 的数量；`referenceOnly` 表示引用可追溯但当前 shader 名不可解析。
...
- `sourceIndexAnimationRelationHealth`：检查 `unity_source_index.db` 是否包含当前工具需要的 Animator/Animation/OverrideController 关系。`AnimatorController -> AnimationClip` 目标解析按 SerializedFile 大小写不敏感匹配，避免 CAB 名大小写差异造成假缺失；如果仍有缺失，会写 `missingAnimatorControllerClipTargetSamples` 作为补 CAB 或重建源索引的具体线索。`explicitControllerClipDomains` 会把显式 controller-clip 关系按 UI 状态机、VFX/特效、预览/Timeline 等通用域粗分；`explicitAnimatorControllerUsages` 会进一步统计 `Animator.m_Controller` 的使用域、Avatar 覆盖和 controller clip 边覆盖。二者都只作为诊断提示，不参与模型-动画绑定，也不能把 UI/VFX/预览 controller 升级成角色生产动画。若素材库里存在 `AnimatorOverrideController`，但源索引没有 `animatorOverrideController.overrideSet` / `clipPair` 精确标记，说明源索引是旧版本或不完整版本；Library 重建会保守跳过这类不完整 OverrideController 的 base controller 粗扩散，应先重建 `unity_source_index.db`，再重建 `library_index.db` 来恢复精确 override 候选。`overrideSet(count=0)` 表示 Unity 明确给出了空替换表，可以确定性继承 base controller 动画。
...
显式动画诊断或旧库迁移后，可以用轻量诊断脚本快速回答“旧模型-动画候选有没有混入猜测关系”：
...
自动验收或批处理里可以加 `-FailOnWarning`。这会在旧动画候选表混入非显式关系，或源索引动画关系健康检查出现 warning 时返回非零退出码；报告仍会写出，方便直接定位原因：
...
原神这类旧动画诊断大库如果只想在重建后快速拦截“候选表混入猜测关系”，加 `-GateOnly`。它只跑候选表和源索引健康门禁，跳过 Unity bake cache、Avatar blocker 和分类覆盖大统计；需要完整覆盖率报告时再去掉这个参数：
...
- `gate.status`：默认应为 `ok`。
- `totals.nonExplicitCandidates`：必须为 `0`，否则说明旧动画候选表混入了非 Unity 显式关系，需要追查是否退回骨骼数量、名称或路径 fallback。
...
这不会影响模型、材质、贴图、验证报告和源索引健康统计。需要完整文件浏览、文件存在性审计或 Library Browser 文件级搜索时，再重新运行不带 `--skip_sqlite_file_index` 的完整建库。

如果只想快速定位大库 SQLite 重建卡点，可以再加 `--skip_sqlite_sidecar_scan`。当前默认动画已 out-of-scope，这个参数主要用于兼容旧库或显式诊断数据：
...
使用这个参数生成的 summary 不应用于判断旧动画诊断覆盖率；默认模型素材库验收不需要这部分统计。

`--skip_sqlite_json_documents` 只跳过把大型 JSON 原文复制到 `json_documents` 表；`assets`、模型验证、材质 sidecar、贴图关系等结构化表仍会正常生成。需要 SQLite 里保留大型 JSON 原文供桌面端全文读取时，再重新运行不带该参数的完整建库。
...
`--verify_source_index` 额外输出 `animationPathHashHealth`，统计 hash-only `AnimationClip` binding 的唯一 `pathHash` 有多少能被任意 `Avatar.m_TOS` 解析，并列出高 binding 数 clip 的最佳 Avatar 覆盖样本。这个报告只用于确认“路径可解析、可继续做定向预览”，不能替代 Animator/Prefab/Controller 显式关系、模型材质验收或动画 glTF 截帧验收。
...
- `assetBundle.containerPreloadRange`：container 对应 preload 范围的压缩记录。小范围会继续逐项写 `containerPreload`；超大范围只写 range，保留 boundedStart/boundedEnd、boundedCount、compactReason 等诊断字段，避免把一个 AssetBundle 展开成千万级重复关系。
...
源索引写入会在 `metadata` 中标记 `buildStatus=building/complete`。如果构建被中断并留下巨大 `.db-wal`，`--list_source_model_candidates` 这类重查询会拒绝执行，并提示删除半成品索引后重建或让 `--build_source_sqlite_index` 完整跑完。正常完成的构建会在批次、SQL 索引创建和最终完成时 checkpoint/截断 WAL；报告里的 `assetBundleContainerPreloadExpandedRelations`、`assetBundleContainerPreloadRangeRelations`、`assetBundleContainerPreloadCompactedRanges` 和 `assetBundleContainerPreloadCompactedSkippedRows` 可用于判断是否发生了超大 container-preload 压缩。
...
这个命令只读取源索引里的确定性 Unity 关系，例如 `Animator`、`SkinnedMeshRenderer`、`renderer.material`、`animator.avatar`、`animator.controller`、`AssetBundle.containerAsset/containerPreload/containerPreloadRange`。它输出 `source_model_candidates.json` 和 `source_model_candidates.csv`，用于选择第一阶段模型 smoke 样本；不会导出素材，也不会创建模型-动画绑定。报告会给 Dialog / Timeline / UI / preview / postmodel / cutscene / ability entity 等候选写 `excludeHint`，这类对象默认只适合索引诊断或显式扩展，不应作为生产模型验收样本。若源索引是当前版本并包含 `assetBundle.containerAsset` / `assetBundle.containerPreload` / `assetBundle.containerPreloadRange` / `resourceManager.container`，候选还会写 `containerPath`，用来提前识别 `dialog/tmpobject` 这类路径语义；旧源索引没有 container 关系时该列会为空，此时必须靠导出后的 `asset_catalog.jsonl` / 输出路径 / 截图继续降级临时对象。只有后续导出的模型通过 `model_validation.json`、glTF 校验和清晰静态截图确认 Mesh、UV、材质、贴图、skin、bbox 正常，才允许进入动画候选和动画预览阶段。

非定向候选扫描在大源索引上采用 bounded scan：先从确定性 container/root 或 Renderer 关系里取有限数量的候选，再对这些候选补材质、贴图和容器统计。container 主候选会进一步限制 broad 探测数量；这是为了给 smoke 选样，不是全库最高分排行榜。这样可以避免在 NoRest / Naraka 这类存在千万级潜在 `containerPreload` 展开的源索引上，为了挑少量 smoke 样本先做全库聚合。需要指定角色、武器或场景时，应优先传 `--preview_model` / `--names` 做定向查询。
...
`-RefreshZhumuVerifiedAnimationPreview` 是旧动画预览刷新开关，当前标准下已废弃。即使显式传入，smoke 也只会记录 `skippedDeprecated`，不会改写 `-ZhumuScriptAnimationSampleRoot`，也不会调用 `--apply_verified_animation_preview` 写回动画资产或模型-动画关系。

`smoke_summary.json.firstUsableReadiness` 是当前一行 smoke 的机器验收入口。它按 `sourceInput`、`assetLibraryV1`、`representativeModels`、`textures`、`materials`、`skeleton`、`staticEnvironmentExtension`、`animationRelation`、`browserPreview` 汇总现有门禁。当前 Naraka 第一版应读作模型素材库可用：模型、贴图、材质、骨骼和索引闭环通过，`animationRelation` 进入 `skippedAreas`，因为动画已退出默认导出目标。若任何非动画 area 变成 `blocked`，说明第一版 smoke 不可交付。

当前本机默认完整复验命令应不带动画刷新开关：`tools\Export-NarakaFirstUsableSmoke.ps1 -OutputRoot "D:\Assets\Naraka\Naraka_FirstUsableSmoke_FinalFull_Current"`。通过标准是：代表模型 `model_validation` 通过、`textureLinkErrors=0`、AssetLibrary Browser 校验和缩略图渲染为 `ok`，代表 Library 保持 `asset_library.json.capabilities.animations=false`、`animationSupport.defaultModelAnimationCandidateCount=0`、`modelAnimationRelations=0`，并在 `animation_out_of_scope.json` / `firstUsableReadiness.skippedAreas=["animationRelation"]` 中记录动画默认跳过。脚本字段里的 AnimationClip PPtr 只作为 `sourceIndexScriptAnimationClipScripts`、`sourceIndexSimpleAnimationClipDomains`、`zhumuScriptAnimationProbe` 等诊断线索，不能再升级成默认 Library 动画能力。

Zhumu 合并动画预览的 `smoke_summary.json.zhumuMergedAnimationPreview.renderSubjectOccupancy` 还会包含 `motion` 摘要，并在顶层预览对象里镜像为 `renderPixelMotion`。它用三帧 PNG 的前景像素差保护“确实有可见帧间变化”，和 `renderBboxMotion`、主体占屏门禁一起防止空帧、远景小图或静态截图误过；但它仍只是诊断字段，不能替代动作自然性、骨骼正确性或生产复用验收。

2026-06-26 quick smoke `D:\Assets\Naraka\Naraka_FirstUsableSmoke_ScriptClipScripts_Quick_Current` 复验了这一点：`sourceIndexScriptAnimationClipScripts.totalRelations=43217`、`distinctClipObjects=17138`、`scriptCount=5`，其中 `SimpleAnimation=20998/8663`、`UIStateAnimator=19320/5820`、`AnimationPlayableAsset=1745/1545`、`AnimationTrack=1044/1044`、`SpeedAnimationPlayer=110/66`；同一摘要仍要求 `productionReadiness=blocked`，并保持 `capabilities.animations=false`、`modelAnimationRelations=0`。

2026-06-26 quick smoke `D:\Assets\Naraka\Naraka_FirstUsableSmoke_SimpleAnimationVisibleSubtree_Quick_Current` 继续补齐 `sourceIndexSimpleAnimationVisibleSubtreeProbes`：它复用已选模型的按需诊断，记录 `mo_pve_b_zhumu_soul_01` 和 `ch_f_japan_yaodaoji_lv_s14_wings` 两个 SimpleAnimation 探针都能追到可见且 skinned 的子树，`visibleSubtreeProbeCount=2`、`skinnedSubtreeProbeCount=2`、`animatorProbeCount=2`。这个字段只说明脚本节点附近有真实可见模型上下文，仍不能把脚本 clip 直接写成生产模型动画关系。

2026-06-26 quick smoke `D:\Assets\Naraka\Naraka_FirstUsableSmoke_SimpleAnimationFieldPaths_Quick_Current` 继续把字段路径语义写进同一个摘要：两个 SimpleAnimation 探针都必须保留 `m_Clip` 和 `m_States.data.clip`，Zhumu 为 `20/20` 行，Yaodaoji wings 为 `2/2` 行；`animationNameSamples` 会列出 Zhumu attack/pre clip 和 Yaodaoji wings idle clip。这个字段用于后续解析 SimpleAnimation 默认 clip 与 states clip 的脚本语义，不等同于已恢复 AnimatorController 状态机。
...
这个命令会写 `naraka_avatar_mesh_export_plan.json` 和 `naraka_avatar_mesh_export_commands.ps1`。计划脚本会复用生成计划时正在运行的 CLI 路径，避免 Debug/Release 版本错位。计划里的 `--source_files` 和 `--path_ids` 只来自源索引确定性关系：`ActorBodyVisualCell.lod0RendererAssistants -> LXRendererAssistant.avatarMeshAsset`、`LXRendererAssistant._renderer`、`renderer.material`、`material.texture`，以及同 SerializedFile 内可追溯的 `SkinnedMeshRenderer`、`Transform` 和 BoneDriver 诊断线索。脚本会完整导出同包 `ActorBodyVisualCell.transformNodes.data` 指向的 Transform 诊断节点，并把大量 PathID 按小批次拆成多条命令，避免命令行过长或后续 TRS/节点诊断被采样截断。脚本会按 source file 分段导出 TypeTree JSON、材质/贴图，再生成诊断 glTF 并重建 `asset_library.json` / `library_index.db`；每个步骤完成后会在 `.naraka_plan_state/` 写 checkpoint，命令中断后可以直接重跑脚本跳过已完成步骤。Naraka 大 bundle 加载可能很慢，profile 里应保留 `load_batch`、`export_batch`、`clear_batch` 证据。

计划器还会导出选中 GameObject 上除 `ActorBodyVisualCell` 之外的附加 MonoBehaviour 组件，并对这些附加组件追一层 `MonoBehaviour` / `Transform` PPtr 目标。这个扩展仍然只来自 Unity `component.gameObject` 和 `monoBehaviour.pptr` 显式关系，作用是保留脸部、表情、换装或运行时配置线索，不会自动升级成完整角色装配、skin 或动画绑定。2026-06-26 复验 `ch_m_face_battle` 输出到 `D:\Assets\Naraka\FaceMaleBattle_AttachedComponentPPtrPlan_Current`：报告写出 `attachedMonoBehaviourComponents.count=2`，包含 `ActorBodyVisualCell` 和 `AvatarFaceRuntime`；`attachedComponentPPtrTargets.count=10`，包含 `m_AvatarFaceData -> ch_m_avatar_data`、`m_RootAnchor`、`m_NeckNode`、`m_HeadNode` 和眼睑/眼球 Transform。最小导出确认 `TypeTreeDump\MonoBehaviour\ch_m_avatar_data.json` 已落盘，里面有 `m_FaceType`、贴花区域和 76 个 `m_AvatarBones` bind pose 记录；这些是后续脸部骨骼/表情还原证据，但当前仍不能绕过 face 模块缺正式 skin 和完整角色装配的门禁。

`ActorBodyVisualCell` 目录模式会读取同一 TypeTreeDump 目录中挂在选中 GameObject 上的 `AvatarFaceRuntime`，并通过 `m_AvatarFaceData` 的 PPtr 追到 `AvatarFaceData.m_AvatarBones`。报告和 catalog 会写 `faceRuntimeEvidenceStatus`、`faceRuntimeAvatarBoneCount`、`faceRuntimeMappedUsedBoneIndexCount` 和 `faceRuntimeEvidence`。这些字段只说明脸部运行时骨骼表、骨骼名和 bind local TRS 已经能和 `AvatarMeshDataAsset.m_AvatarBoneWeights` 的 boneIndex 范围对齐；在 boneIndex 到 glTF joint 层级、inverse bind matrix 和清晰视觉验证完成前，仍不能写生产 skin。

需要临时观察这张脸部运行时骨骼表是否能驱动 `ch_m_face_battle` 这类自定义脸网格时，可以追加 `--naraka_avatar_mesh_face_runtime_skin_diagnostic`。这个开关只在默认关闭的诊断路径里，把 `AvatarMeshDataAsset.m_AvatarBoneWeights` 按 `AvatarFaceRuntime.m_AvatarFaceData -> AvatarFaceData.m_AvatarBones` 顺序写成 glTF `JOINTS_0` / `WEIGHTS_0` 和 skin，并用 `m_AvatarBones` 的 bind local TRS 层级反算 inverse bind matrices。报告会写 `faceRuntimeDiagnosticSkin`，验证和动画门禁仍保持 warning/blocked，例如 `face_runtime_diagnostic_skin_present`、`face_runtime_bind_local_inverse_matrices_diagnostic`、`diagnostic_custom_mesh`。它只能用于排查脸部 boneIndex 顺序、bind pose 空间和静态形变，不能作为完整角色 skin、正式模型验收或动画 smoke 通过证据。

计划器还会额外导出全局发型 customization/tint 配置 JSON，例如 `SpecialHairCustomColorData`、`SpecialHairCustomPartDecorator` 和 `HairCustomConfigAsset`。这些文件能证明 Naraka 存在 `HighLightingColor`、`Ks`、渐变分区等可解释字段；但在找到和选中 `ActorBodyVisualCell` / `GameObject` 的确定性 PPtr 或其它 Unity 结构引用前，只能作为材质研究证据，不能自动烘焙到当前发型 glTF。

如果同名 GameObject 有多个实例，计划器只会选择能完整连到 `lod0RendererAssistants -> avatarMeshAsset` 的 `ActorBodyVisualCell`。只有 `lod0RendererAssistants` 但没有 `avatarMeshAsset` 的对象会被跳过，避免把同名诊断对象误当成可导出的 Naraka 自定义网格。
...
从 `TypeTreeDump/MonoBehaviour` 子目录单独复测时，工具会向上查找同计划根目录的 `export_manifest.jsonl`、`Material/` 和 `Texture2D/` 作为预览材质证据，并把 glTF 实际引用的贴图复制到本次输出目录。这个行为只是复用已经确定性导出的资源，不会按名称猜贴图、发色或 shader 关系。

`ActorBodyVisualCell` 目录模式还会在 `--preview_output` 根目录写入 `asset_catalog.jsonl` 和 `model_validation.json`，方便继续运行 `--build_sqlite_index <preview_output> --source_index <unity_source_index.db> --game Naraka` 生成 AssetLibrary v1 入口。这里的 catalog 记录使用 `libraryRole=CustomMeshDiagnostic`、`resourceKind=NarakaCustomMeshPart`，验证状态固定为 `warning`；同目录真实导出的 `Texture2D/*.png` 会作为 `assets.kind=Texture` 写入 catalog，并标注是否被 glTF 标准预览材质引用。它只让浏览器和 SQLite 能定位这份诊断 glTF 与贴图，不表示模型已经具备 skin、完整角色装配、发色 customization 或动画验收资格。

这类 catalog 会从 `selectedVisualCellGameObjectName` 写出保守的 `characterAssemblyRole`、`characterAssemblyFamily`、`characterAssemblyCandidateOnly=true` 和 `characterAssemblyBasis=selectedVisualCellGameObjectName`。例如 `ch_m_hadi_hair_lv_ss0` 会标成 Hair / `ch_m_hadi`，方便浏览器和 `character_assemblies.json` 显示候选；但该字段仍是诊断索引，不是 Unity 正式装配关系。

重建 `library_index.db` 后，`assets` 表会把 `library_role`、`diagnostic_only`、`character_assembly_role`、`character_assembly_family`、`character_assembly_candidate_only`、`material_status` 和基础材质布尔值展开为显式列。统一浏览器或人工 SQL 可以直接筛选这些字段，不必每次从 `raw_json` 解析；但它们仍只继承 catalog 的诊断语义，不能绕过 `diagnosticOnly`、模型验证和动画门禁。`character_assembly_candidate_only` 也会被 `character_assemblies.json` 生成器当作人工候选门禁，不进入默认自动推荐。

报告和 catalog 会额外写 `sourceSkinAvatarMaxBoneIndex`、`sourceSkinAvatarRequiredNodeCount`、`selectedVisualCellTransformNodeRequiredCount`、`selectedVisualCellTransformNodeMissingCount`、`selectedVisualCellTransformNodeCoverageStatus` 和 `sourceSkinMappingStatus`。这些字段只说明选中 `ActorBodyVisualCell.transformNodes.data` 是否足够覆盖 `AvatarBoneWeights` 的最大 boneIndex，以及当前为什么仍不能写 glTF skin；即使选中 VisualCell 没有节点表，需求数也会从 mesh 权重本身汇总出来。同一个 SerializedFile 里其它 VisualCell 的节点表即使覆盖范围，也只能作为诊断线索；没有确定的 joint 名称/路径映射前，不能拿来绑定选中部件。

每个 mesh 的 `skin.bindPoseMatrix` 会汇总 `m_BindPoses` 的矩阵数值健康度，包括有限值、仿射矩阵、近零 determinant 和少量样本行。这个字段只帮助判断源 bind pose 是否可读；它不证明 `AvatarBoneWeights.boneIndex` 已经能映射到 Unity joint，也不能单独放行 glTF skin、正式装配或动画验收。

每个 part 的 `rendererBonesAnimSkinEvidence` 会把 `SkinnedMeshRenderer.m_Bones` 的显式 Unity PPtr、`AvatarMeshDataAsset.m_AnimSkinData` 的局部 boneIndex 和 `m_BindPoses` 槽位做一一对应诊断。`rendererBonesMatchAnimSkinBindPoseSlots` 只说明这条 Renderer-bones 局部 skin 链路值得继续验证；在 inverse bind matrix 空间、joint 层级和静态/动画视觉验收通过前，仍不能写生产 skin 或让动画 smoke 通过。

需要临时观察 Renderer-bones 局部 skin 是否能保持 Naraka face 这类自定义网格静态形状时，可以追加 `--naraka_avatar_mesh_renderer_bones_skin_diagnostic`。这个开关只在默认关闭的诊断路径里，为每个 part 单独写 glTF skin：joint 顺序来自 `SkinnedMeshRenderer.m_Bones`，权重来自 `AvatarMeshDataAsset.m_AnimSkinData`，inverse bind matrix 来自转置后的 `m_BindPoses`。为了避免使用尚未证明匹配的 Unity Transform 层级导致静态拉飞，诊断 joint rest 矩阵先由 `inverse(m_BindPoses)` 反推；报告会写 `rendererBonesDiagnosticSkins[*].restSelfCheck`，用 `jointRestMatrix * inverseBindMatrix ~= identity` 记录静态 rest 是否自洽；同时写 `transformHierarchyMatrixCheck`，按 Renderer bone 槽位比较导出的 Transform 世界逆矩阵、rootBone 相对逆矩阵和 `m_BindPoses` 是否接近，并用 `spaceSummaries` 汇总各矩阵空间误差。验证和动画门禁仍保持 warning/blocked，例如 `renderer_bones_diagnostic_skin_present`、`renderer_bones_transposed_bind_pose_inverse_matrices_diagnostic`。它只能证明局部权重和 bind pose 槽位自洽，不能作为完整角色 skin、正式装配或动画 smoke 通过证据。

`--export_naraka_avatar_mesh_plan` 会额外从 `SkinnedMeshRenderer.bones` / `rootBone` 出发，沿 `transform.parent` 显式关系导出 Renderer bone 父链 Transform，并在计划 JSON 写 `rendererBoneTransformExport`。这只补齐 `transformHierarchyMatrixCheck` 的输入证据；即使父链完整，仍要看矩阵误差、静态视觉和模型门禁，不能把它当作生产 skin 放行。

`transformNodeTableCandidates[*].bindPoseTransformMatrixMatch` 会把 `m_BindPoses` 的槽位矩阵与候选 `transformNodes.data` 的 Transform 逆世界矩阵做最近匹配诊断，同时分别尝试 raw/transposed bind pose 和 Unity/glTF 坐标空间矩阵。它只能提示后续该重点检查哪些节点或矩阵约定；矩阵接近不等于 Unity 显式 joint 映射成立。

每个 `transformNodeTableCandidates` 还会写 `usedBoneIndexCount`、`coveredUsedBoneIndexCount`、`missingUsedBoneIndexCount`、`coversAllUsedBoneIndices`、`coveredUsedBoneRefs`、`missingUsedBoneRefs`、`exportedCoveredUsedBoneTransformCount` 和 `allCoveredUsedBoneTransformsExported`。这些字段只检查 `AvatarBoneWeights` 实际参与权重的非负 boneIndex 是否能按节点表顺序落到 `transformNodes.data`，以及对应 Transform JSON 是否已导出并能读到本地 TRS；它们比只看最大 boneIndex 更适合判断下一步该追哪张节点表，但仍不是 glTF skin 的放行条件。报告会优先列出覆盖度最高的候选，同时保留本次选中 VisualCell 的候选，方便直接看到选中表缺哪些 boneIndex。候选里还会写 `selectedVisualCellIndexOrderComparison`：如果同一个 boneIndex 在其它覆盖表和选中表中节点名大量冲突，则说明这些表不是同一套局部骨骼顺序，不能把其它表当成选中网格的 skin joint 补丁。

需要临时观察外部节点表是否能驱动 Naraka 自定义网格时，可以给同一个 `--export_avatar_mesh_data_gltf` 目录模式追加 `--naraka_avatar_mesh_external_skeleton_skin_diagnostic`。这个开关只在默认关闭的诊断路径里，把 `m_AvatarBoneWeights` 按覆盖度最高且已导出 TRS 的外部 `transformNodes.data` 候选写成 glTF `JOINTS_0` / `WEIGHTS_0` 和 skin，并使用外部骨架 rest world 矩阵反算 inverse bind matrices。报告会写 `externalSkeletonDiagnosticSkin`，验证和动画门禁仍会保留 warning/blocked，例如 `external_skeleton_diagnostic_skin_present`、`external_skeleton_rest_inverse_bind_matrices_diagnostic`、`selected_visual_cell_transform_nodes_missing`。这只能用来排查 boneIndex 顺序、Transform 层级和静态变形，不能作为生产 skin、正式模型验收或动画 smoke 通过证据。

2026-06-26 复验 `ch_m_hadi_hair_lv_ss0`：`D:\Assets\Naraka\HadiHairSS0_CurrentDebugPlan` 通过 `--export_naraka_avatar_mesh_plan` 和计划脚本生成了 `MonoBehaviour_lod0.gltf`。默认诊断 glTF 有 5 个 mesh、6879 顶点、3 张预览贴图，glTF validator 无 error/warning，但 `model_validation.status=warning`，`sourceSkinMappingStatus=blockedSelectedTransformNodesMissing`，`selectedVisualCellTransformNodeCoverageStatus=missingSelectedVisualCellTransformNodes`，材质仍为 `needsCustomizationTint`。同样输入追加 `--naraka_avatar_mesh_external_skeleton_skin_diagnostic` 输出到 `D:\Assets\Naraka\HadiHairSS0_ExternalSkinDiag_CurrentDebug` 后，glTF validator 仍无 error/warning，报告写出 1 个诊断 skin、20 joints，并确认外部 `ch_m_hadi_lv_s9` VisualCell 覆盖 hair 实际使用的 boneIndex；但门禁继续阻断，原因包括 `external_skeleton_diagnostic_skin_present`、`external_skeleton_rest_inverse_bind_matrices_diagnostic`、`selected_visual_cell_transform_nodes_missing` 和 `needs_customization_tint`。截图显示 hair 几何可见但仍是灰色/tint 未复原；该样本只能作为 Naraka 自定义发型 mesh/skin 映射研究证据，不能作为完整角色、正式 skin 或动画 smoke。

2026-06-26 复验 `ch_m_face_battle`：源索引里 `ch_m_face_battle` / `ch_f_face_battle` 都有 `ActorBodyVisualCell -> lod0RendererAssistants -> avatarMeshAsset` 确定性链路，LOD0 指向 `eyelash_mesh_data`、`eyes_mesh_data`、`face_mesh_data`。`D:\Assets\Naraka\FaceMaleBattle_CurrentDebugPlan` 通过计划脚本生成的 `MonoBehaviour_lod0.gltf` 有 3 个 mesh、3277 顶点、3 个材质和 5 张 glTF 预览贴图，validator 无 error/warning，截图能看到脸、耳朵、眼睛和睫毛贴图。追加附加组件计划后，`D:\Assets\Naraka\FaceMaleBattle_FaceRuntimeEvidence_Current` 复验报告写出 `faceRuntimeEvidenceStatus=avatarFaceDataMatchesSourceSkinBoneTable`、`faceRuntimeAvatarBoneCount=76`、`faceRuntimeMappedUsedBoneIndexCount=73`，说明实际用到的脸部 boneIndex 都能落到 `ch_m_avatar_data.m_AvatarBones` 名称表里。显式追加 `--naraka_avatar_mesh_face_runtime_skin_diagnostic` 输出到 `D:\Assets\Naraka\FaceMaleBattle_FaceRuntimeSkinDiag_Current` 后，报告写出 `diagnosticSkinStatus=faceRuntimeDiagnosticSkin`、`skinCount=1`、`boneCount=76`，glTF validator 无 error/warning，F3D 静态截图未见 bbox 拉爆；但 `model_validation.status=warning` 和动画门禁继续 blocked，因为该 skin 的 inverse bind matrix 仍是脸部 `AvatarFaceData` bind local TRS 诊断结果，缺完整角色父骨、材质 tint 和清晰视觉验收。该样本仍是 face 模块而非完整角色，不能进入动画 smoke。

face runtime 报告里的 `faceRuntimeEvidence.bindLocalTrsSelfCheck` 会检查 `AvatarFaceData.m_AvatarBones` 的 bind local TRS 层级是否和诊断 inverse bind matrix 自洽。`bindLocalTrsSelfConsistent` 只说明 face runtime 这条局部骨骼路线内部矩阵可用；缺完整角色父骨、材质 tint、装配关系和清晰视觉验收前，仍不能升级为生产 skin。

如果 face runtime skin 诊断输出变成灰模，先检查同一计划输出目录是否已经完整跑完 `naraka_avatar_mesh_export_commands.ps1`。`--export_avatar_mesh_data_gltf` 目录模式只会复用同级或上级已有的 `Material/*.json`、`Texture2D/*.png` 和 `export_manifest.jsonl`，不会在单独重导 skin 时自动补材质。2026-06-26 复验 `D:\Assets\Naraka\FaceMaleBattle_AttachedComponentPPtrPlan_Current`：计划脚本跑完后再把 `TypeTreeDump\MonoBehaviour` 导出到 `D:\Assets\Naraka\FaceMaleBattle_FaceRuntimeSkinMaterial_Current`，报告写出 `previewMaterialSource=D:\Assets\Naraka\FaceMaleBattle_AttachedComponentPPtrPlan_Current`、`rendererMaterialRefCount=3`、`materialTextureRefCount=17`、3 个 mesh、3 个材质、5 张 glTF 预览贴图、1 个 diagnostic skin 和 76 个 joint。`model_validation.json` 仍是 `Status=warning`、`animationGate.status=blocked`，原因包括 `face_runtime_diagnostic_skin_present`、`face_runtime_bind_local_inverse_matrices_diagnostic` 和 `selected_visual_cell_transform_nodes_insufficient`；这说明材质预览和诊断 skin 可以同时复现，但仍不能升级成正式 skin 或动画 smoke。

2026-06-26 复验 Hadi body + face + hair 装配预览：以 `D:\Assets\Naraka\Smoke_ch_m_hadi_body_current_debug\model_animations.json` 里的 `ch_m_hadi_lv_s9` 为 base，显式追加 `D:\Assets\Naraka\FaceMaleBattle_CurrentDebugPlan\MonoBehaviour_lod0.gltf` 和 `D:\Assets\Naraka\HadiHairSS0_CurrentDebugPlan\MonoBehaviour_lod0.gltf`，输出到 `D:\Assets\Naraka\HadiBodyFaceHair_AssembledPreview_Current`。`assembly_report.json` 正确把两个同名 `MonoBehaviour_lod0.gltf` 分别标为 `Face` / `Hair`，并写出 `DiagnosticUnskinned=true`：face 新增 3 个 mesh node，hair 新增 5 个 mesh node；最终 glTF 有 28 个 mesh、20 个 skin、`invalidSkinJoints=0`、`invalidChannels=0`，validator 无 error/warning。截图显示 hair 和 face 坐标都落在头部范围，但它仍是显式诊断装配预览，face/hair 没有正式 skin 绑定，不能替代 `character_assemblies.json` 的确定性 joint remap，也不能进入动画 smoke。
...
导出后的 `model_validation.json` 只验证 glTF 静态结构、Mesh、UV、贴图引用、skin 和 bbox。Naraka 这类 ColorMask/Tint 材质如果只达到 `materialStatus=tintParametersOnly` 或仍有 `materialNeedsCustomizationTint=true`，会在 `model_animations.json.modelAnimationGate` 中追加 `material_customization_tint_not_ready`，阻断默认动画/生产 smoke。这样可以保留可用的诊断 glTF 和材质 sidecar，同时避免把未完成预览材质的角色部件误当作动画验收样本。

材质预览会优先把 `_MainTex` / `_BaseMap` / `_BaseColorMap` 这类主色槽作为 glTF `baseColorTexture`，不会让 `_OutlineDiffuseTexture`、detail 等辅助贴图覆盖主贴图。`ColorMaskTint` 只接受名字明确像颜色的 tint 参数；`_PrimaryWindParam`、`*Param`、`*Tile`、`*Fresnel` 这类 shader 控制向量会保留在 Unity 材质 extras 中，但不会当作配色烘焙。
...
这个命令按已选模型收窄查询，只列 `GameObject -> Animator -> AnimatorController -> AnimationClip`、legacy `GameObject -> Animation -> AnimationClip`，以及保守的 `MonoBehaviour config -> model + Avatar <- Animator.controller -> AnimationClip` 这类 Unity 显式关系，输出 `source_model_animation_candidates.json` 和 `source_model_animation_candidates.csv`。它不会导出动画，不会写 `model_animations.json`，也不会把候选标成可播放。查询顺序必须从已选 GameObject 出发，再查它自己的 Animator / Animation 组件或同源配置资产，避免在大源索引里为了单模型关系诊断全库扫描 Animator。报告里的 `modelReadyForAnimation=false` 是刻意保守的：源索引只能证明引用关系存在，不能证明模型 Mesh、UV、材质、贴图、skin 或 bbox 已经合格。同名 GameObject 可能有多个 prefab、UI、postmodel 或诊断变体；生产验证应优先使用已经通过静态模型验收的具体 pathId，或把同名结果先和 `source_model_candidates.json` / `model_validation.json` 对齐后再进入动画。报告还会写 `modelVisibilityDiagnostics`，快速统计选中 GameObject 及其直接子节点的 MeshRenderer、SkinnedMeshRenderer、Animator、Avatar 和 Controller 引用数量；`visibleModelLike=false` 说明它更像 skeleton/config 诊断对象，不应作为动画 smoke 模型。这个字段是轻量提示，不是完整深层层级扫描，不能替代导出后的模型验证。报告还会写 `animatorDiagnostics`，列出匹配 GameObject 是否有 Animator、Avatar、Controller 和 Controller clip；这只解释为什么显式候选为 0，例如 Endfield 常见的“同名模型有 Avatar 但 AnimatorController 为空”，不能把同名 Animator、Avatar 或骨架兼容升级成动画关系。报告还会写 `monoBehaviourPPtrDiagnostics`，列出哪些 MonoBehaviour PPtr 字段指向选中模型，以及同一个 MonoBehaviour 还指向哪些 Avatar、Prefab、Clip、Controller 或其他对象；这类字段是确定性的配置线索，可以帮助追查角色数据链路，但不能单独升级为可播放动画候选。报告还会写 `scriptAnimationComponentDiagnostics`，检查选中 GameObject 和直接子节点上 MonoBehaviour 组件的 AnimationClip PPtr 字段，并在每行 `gameObject.subtree` 中附带受限深度后代的 `visibleRendererCount`、`skinnedMeshRendererCount`、`visibleGameObjectSamples` 和 `truncated` 摘要；它用于定位 Naraka 这类脚本动画入口和附近可见对象上下文，但限制为本地、按需、有界诊断，不能替代完整层级扫描，也不能把脚本语义或子树 Renderer 邻近关系升级为默认模型-动画候选。报告还会写 `avatarTosClipDiagnostics`，用选中模型自己的 `Animator.avatar -> Avatar.m_TOS` 查表检查 hash-only `AnimationClip` binding 的 pathHash 覆盖率；这是 Naraka 这类动画路径恢复的结构诊断，只写 JSON，不写 CSV 候选，不能替代 AnimatorController 上下文、模型验收、TRS 写回或动画截帧验收。若源索引包含当前版本的 `assetBundle.containerAsset` / `assetBundle.containerPreload` / `resourceManager.container`，报告会在命中模型后回填 `model.containerPath`，并把 Dialog / Timeline / UI / cutscene / tmp / preview 等容器或路径语义写入 `reviewHint`；旧源索引缺 container 关系时该字段可能为空，需要先用 `--verify_source_index` 确认是否应该重建源索引。这类候选最多作为诊断线索；只有模型静态验收通过、动画 sidecar/曲线可稳定解码、并且清晰截帧证明动作合理后，才能进入正式动画 smoke 或 `relation_animations` 推荐结果。

报告里的 `selectedModels` 是 selector 实际命中的源模型，`models` / `candidateModelCount` 是已经找到显式动画候选的模型。Naraka 的 `actor_visual_part` 身体、发型、附件常是可见零件：它们可以模型验收通过，但自己没有 Animator/Controller；这时 `selectedModelCount>0`、`candidateModelCount=0` 是正常诊断结果，不能把独立动画 Clip 按名字硬绑定到这些零件上。

2026-06-26 复验当前 Naraka 源索引 `D:\Assets\Naraka\SourceIndex_Full_HeaderFix1\unity_source_index.db`：索引里有 `AnimationClip=77754`、`Animator=23402`、`Avatar=2013`，但显式 `AnimatorController` 很少，`animator.controller=213`、`animatorController.clip=98`。`ch_m_hadi_lv_s9` 报告输出到 `D:\Assets\Naraka\SourceModelAnimations_HadiBody_Current`，命中 8 个同名/特效变体，0 条候选，主 body prefab 可见但无 Animator；`ch_m_face_battle` 输出到 `D:\Assets\Naraka\SourceModelAnimations_FaceMaleBattle_Current`，命中 7 个 face 变体，0 条候选，其中一个可见 face prefab 有 Avatar 但 `controllerCount=0`；`ch_evacuate_camp_npc_fengluo_skeleton` 输出到 `D:\Assets\Naraka\SourceModelAnimations_NpcSkeleton_Fengluo_Current`，有 Avatar 但 `visibleModelLike=false` 且无 Controller，只能作为 skeleton/config 诊断。对照查询 `device_bigtree_01` 和 `fx_female_fashion_customize_idle_jiyingying_class04_02_around` 也只得到 `controller_without_clip` 诊断。真正能列出 `animatorController.clip` 的 `Btn_FaceRename` 输出到 `D:\Assets\Naraka\SourceModelAnimations_UiBtnFaceRename_Current`，有 15 条 `Disabled/Highlighted/Normal/Pressed` 等 UI clip 候选，`reviewHint=ui`；这证明源索引显式关系查询可用，但 UI clip 不能作为角色、道具或动画 smoke 证据。当前结论是：Naraka 角色 `actor_visual_part` 模块仍停在模型/装配/材质门禁阶段，不能按名字、Avatar 或独立 Clip 推断生产动画绑定；后续要继续找游戏里真正把完整角色 prefab/配置、Avatar、Controller/Clip 串起来的 Unity 显式关系，或先把 custom mesh 的正式 skin、tint 和完整角色装配补齐后再进入动画阶段。

`source_model_animation_candidates.json.scriptAnimationComponentDiagnosticSummary` 是 `scriptAnimationComponentDiagnostics` 的机器摘要，会汇总脚本名、字段路径、clip 数、局部/子树可见 Renderer、Animator 和截断数量。它只用于快速判断 MonoBehaviour -> AnimationClip PPtr 诊断域的形态；即使 `rowCount` 大于 0，`defaultCandidateCount` 也必须保持 0，不能绕过脚本语义、模型验证、TRS 写回和视觉验收。摘要会写 `productionReadiness=blocked` 和 `blockedProductionRequirements`，把缺少脚本语义、模型验证、TRS 导出和视觉验收这些生产门槛结构化保留下来，方便 smoke 不解析说明文字也能挡住误升级。

Naraka 一行 smoke 还会写 `smoke_summary.json.sourceIndexScriptAnimationClipScripts`，从完整源索引聚合所有 `MonoBehaviour.script -> monoBehaviour.pptr -> AnimationClip` 的脚本关系。这个字段用来快速区分“没有生产 AnimatorController 绑定”和“脚本字段里确实有大量 clip 引用”：`SimpleAnimation` 属于下一步重点求解探针，`UIStateAnimator`、`AnimationPlayableAsset` / `AnimationTrack` 和 `SpeedAnimationPlayer` 先保持 UI、Timeline 或脚本运行时诊断域。它同样固定 `productionReadiness=blocked`，不能把脚本关系直接写入 `model_animations.json` 或 `relation_animations`。

同一个 smoke 还会写 `smoke_summary.json.sourceIndexSimpleAnimationVisibleSubtreeProbes`，把 Zhumu 和 Yaodaoji 这类已选 SimpleAnimation 样本的可见子树、SkinnedMeshRenderer 和 Animator 上下文集中列出来。这个字段是为了证明“脚本动画线索旁边有可见模型上下文”，不是为了创建默认关系；若后续要把它推进到生产动画，仍必须先解析 SimpleAnimation 的状态/clip 语义，确认具体模型根和 Avatar，导出 glTF TRS，并做清晰截帧验收。

`sourceIndexSimpleAnimationVisibleSubtreeProbes.probes[*].fieldPathCounts` 和 `animationNameSamples` 是下一步 SimpleAnimation 语义解析入口。当前 smoke 要求每个可见子树探针都同时出现 `m_Clip` 与 `m_States.data.clip`，用来保护“默认 clip + states clip”两类字段不被后续查询退化。消费端只能把它当作脚本字段证据和求解选点，不能按字段名直接生成默认动画候选。
...
`scriptAnimationComponentDiagnosticSummary.simpleAnimationSemanticSummary` 是 SimpleAnimation 字段语义摘要。它只识别 Unity SimpleAnimation 公开实现里稳定的两类字段：`m_Clip` 作为默认 clip，`m_States.data.clip` 作为 state clip，并统计 `pairedDefaultStateClipRows`，表示同一个 SimpleAnimation 组件的默认 clip 和 state clip 是否指向同一个 AnimationClip。若源索引带有 TypeTree 摘要，还会写 `automaticDefaultStateClipRows` 和 `automaticDefaultStateClipSamples[]`，表示该组件同时满足 `m_PlayAutomatically=true`、唯一 default state、TypeTree `m_Clip` 和 state clip 指向当前同一个 AnimationClip。这个字段可以帮助判断下一步该把哪个 clip 放进 Humanoid/TRS 求解探针，但仍不能说明运行时代码没有切换 state，也不能创建生产动画关系。

`simpleAnimationSemanticSummary.pairedClipSamples[]` 会列出最多 8 条同组件 default/state 配对样本，包含模型、GameObject、MonoBehaviour、AnimationClip 的 source、PathID、`roles`、`diagnosticOnly=true` 和 `notDefaultModelAnimationRelation=true`。PathID 同时提供数字字段和 `*PathIdString` 字段；浏览器、Node.js 或其它 JavaScript 消费端应优先使用字符串字段，避免 64 位整数精度丢失。这些样本只用于后续定向 Humanoid/TRS 求解探针，不允许直接写入 `model_animations.json`、`relation_animations` 或把 `capabilities.animations` 改成 `true`。

新版源索引会在 `source_objects.raw_json.monoBehaviour.simpleAnimation` 中为 `SimpleAnimation` 额外保存轻量 TypeTree 摘要：`playAutomatically`、`m_Clip`、`stateCount`、`defaultStateCount`、`stateNames`、`defaultStateNames`、`stateClipPathIds` 和 `typeTreeStatus`。`--list_source_model_animations` 会把它上浮到 `scriptAnimationComponentDiagnostics[*].monoBehaviour.simpleAnimationTypeTree`、`simpleAnimationSemanticSummary.typeTreeMetadataRows`、`typeTreePlayAutomaticallyRows`、`typeTreeDefaultStateRows` 和 `automaticDefaultStateClipRows`。这能把 Naraka 的脚本动画线索从“字段 PPtr”推进到“自动播放默认 state -> 确定 clip”证据；但它仍只说明脚本配置，不代表运行时代码一定播放该 state，也不代表 Humanoid/TRS 求解和视觉验收通过。旧 `unity_source_index.db` 没有这些字段时会显示 `typeTreeMetadataNotIndexedRows`，应重建源索引后再判断状态元数据覆盖率。

旧全量源索引也可以用显式维护命令只回填这段 TypeTree 摘要，不必重建几十 GB 的 `unity_source_index.db`：

```powershell
AnimeStudio.CLI.exe `
  --refresh_simple_animation_typetree "D:\Assets\Naraka\SourceIndex_Full_HeaderFix1\unity_source_index.db" `
  --preview_source_root "C:\Game163\program\NarakaBladepoint_Data\StreamingAssets" `
  --source_files "4\f\4fd035612d51ea0f" `
  --preview_output "D:\Assets\Naraka\SimpleAnimation_TypeTreeRefresh_ZhumuFullIndex_Current"
```
...
根目录 `LIBRARY_README.md` 会写出“动画边界”：默认动画导出已关闭，正式动画资产数量和默认模型-动画候选数量都应为 0。README 会明确说明这不影响模型、骨骼、贴图和材质作为静态素材使用。

SQLite 也会写同一层机器状态：`metadata.key='animationSupport'` 和 `sqlite_index_summary.json.animationSupport`。字段包括 `status`、`productionReady`、`animationAssetCount`、`defaultModelAnimationCandidateCount`、`modelGateReadyCount`、`modelGateBlockedCount`、`modelReadyForAnimationCount` 和 `blockedReasonCounts`。浏览器或自动化应优先读这个字段区分“当前无生产动画”与“索引缺失/导出失败”，而不是只看 `assets.kind='Animation'` 是否为空。
...
一个模型可能在游戏里关联很多动画，尤其是角色资源：私有表情动画、角色展示动画、通用身体动作、职业/性别动作、Animator Controller 引用动作、外部公共动作库等。当前默认 Library 不再导出这些动画，也不生成模型-动画候选；默认目标先收稳模型、贴图、材质、骨骼/skin、来源关系和 AssetLibrary v1 索引。
...
2. 动画默认跳过。
   AnimationClip、Timeline、材质动画、摄像机动画和 VFX 动画默认只作为 out-of-scope 诊断线索，不进入 `Animations`。
3. 自动生成 Unity 关系图和模型索引。
   `unity_relations.jsonl` 记录 Unity 原生关系，`unity_relation_summary.json` 给出轻量摘要；`animation_out_of_scope.json` 负责说明动画已按默认规则跳过。
4. 模块化角色保持模块化。
...
`--assembly_modules` 也可以显式传入一个 `.gltf` / `.glb` 文件路径，例如 Naraka `--export_avatar_mesh_data_gltf` 生成的 face 诊断 glTF。显式路径允许追加“有 mesh、无 skin”的诊断模块，用来检查坐标、材质和可见装配效果；报告会标记 `DiagnosticUnskinned=true`，不能把它当作正式 prefab 绑定、skin 绑定或动画验收证据。显式模块的 Face/Hair/Accessory 角色会从文件名和父目录一起推断，避免 Naraka 这类都叫 `MonoBehaviour_lod0.gltf` 的诊断模块互相覆盖预览 buffer 或贴图。默认按角色自动选择的模块仍必须通过同名 joint 映射。
...
显式动画诊断导出或旧库迁移时，`asset_catalog.jsonl`、`animation_bindings.jsonl`、`model_animations.json` 会带上 `animationAsset` 路径，后续预览、打包或 Unity/Blender 转换器都应该优先读取这个 sidecar，而不是靠动画文件名猜测语义。默认 Library 不生成这些动画索引。
...
合并成功时，报告会写 `animationJointCoverage`，统计动画 channel 直接命中的 skin joint 数、总 skin joint 数、覆盖率和已/未驱动骨骼样本；其中 `vertexWeightCoverage` 会继续按 mesh 的 `JOINTS_0` / `WEIGHTS_0` 统计已驱动 joint 对顶点权重的覆盖，`coreBodyCoverage` 会按通用人体主体骨词元统计 Pelvis/Spine/Head/Arm/Leg/Foot/Toe 等核心骨骼覆盖，并排除 finger/twist/helper/hair/socket 等辅助骨，`rootMotionCoverage` 会统计未直接命中 skin joint、但目标节点是 skin joint 祖先的 TRS channel，用来解释 RootT/RootQ 等 root motion 是否落在骨架容器上。这个字段只帮助判断当前 standalone 动画实际覆盖到哪些骨骼、多少可见网格权重，以及 root motion 的诊断落点；它不是视觉验收，也不能单独把 Humanoid/Muscle 或 UnityBakeAccelerated 结果升级为生产可用。
...
- 默认导出阶段只把动画当作 out-of-scope 线索，不导出、不嵌入、不生成默认候选。
...
- 只有显式动画诊断、旧库迁移或专门预览/打包命令，才允许处理动画；这些输出不能混入默认 AssetLibrary 结论。
...
- `animation_bindings.jsonl`、`model_animations.json` 和 `model_animations.compact.json` 是旧诊断/迁移索引，不属于默认 Library 输出。默认索引用 `animation_out_of_scope.json` 说明动画已跳过。
- 显式动画诊断仍必须记录 `relationSource`、`confidence`、`matchedBindingPaths`、`unmatchedBindingPaths` 等证据，且不能把结构兼容、骨骼路径、Avatar 兼容、路径/名称语义升级成默认 Library 关系。
- 预览/打包命令如果存在，必须自己验证实际写入 glTF 后的 channel 数、skin/joint 和主体骨骼覆盖，不能借默认素材库验收通过。
...
- `Models` 里的角色/蒙皮模型会保留 skeleton/skin；骨骼属于模型数据，不代表默认导出动画。
- 默认不会生成正式 `Animations` 资产目录；动画资源只作为 out-of-scope 诊断线索记录。
...
- `asset_catalog.jsonl` 记录模型、贴图、材质、实验 shader 的结构化索引和模型统计。
- `asset_summary.json` 汇总导出数量、资源分类和模型统计。
- `unity_relations.jsonl` 记录 Unity 原生关系，是后续从模型定位 Avatar、Material、Texture、Mesh 关系的主入口。
- `unity_relation_summary.json` 汇总关系数量和关键覆盖率，便于快速排查模型、材质和依赖闭包问题。
- `model_validation.json` 验证 glTF 模型、贴图、材质和 skin/joint 基础结构。
- `animation_out_of_scope.json` 说明默认跳过动画资源；它不是动画支持报告。
...
现在默认素材库导出会避免这种失败形态：模型入 `Models`，动画不入默认库，也不把全局动作库重复塞进每个角色模型。
...
- `Animator`：按 Animator 导出，只适合显式诊断旧式动画收集，不作为默认素材库路径。
...
`asset_catalog.jsonl` 面向素材库浏览和自动验收。模型条目会记录资源分类、mesh 数、顶点数、材质数、贴图数、骨骼数、`skeletonHash` 和 `skeleton`。`skeletonHash` 是素材库骨架 ID；`skeleton` 里会记录 `namePathHash`、`hierarchyHash`、`bindPoseHash`、`avatarHumanHash`、`avatarSkeletonNameHash` 和 `relationBasis`，用于判断模型自身骨架/skin 是否自洽。Humanoid/Avatar 相关字段只能作为模型骨骼资料和显式诊断输入，不代表默认导出动画。
...
默认 Library 不再生成 AnimationClip catalog 条目。显式诊断/旧库迁移命令如果生成动画 sidecar，必须把它和正式 AssetLibrary 分开看待。

`asset_summary.json` 是总览报告，记录模型、贴图、材质、实验 shader 的数量和模型是否带贴图、骨骼、morph。`model_validation.json` 是静态模型验收报告，检查 glTF image、texture、material、mesh accessor、skin joint、inverseBindMatrices 是否自洽。`animation_out_of_scope.json` 说明动画默认跳过。`character_assemblies.json` 记录模块化角色的 body/face/hair/accessory 候选和推荐组合；它只描述可组装关系，不改变默认 Library 中的原始模块输出。

显式动画诊断或旧库迁移如果生成 `model_animations.json` / `model_animations.compact.json`，它们只表示下一步安全处理路径，不等于动画已经视觉验证通过。候选里的 `requiresUnityBake=true` 是旧库兼容字段，只能说明历史流程可能生成外部 Unity bake 请求，不能作为生产可播放证明；这些文件不属于默认 Library 输出，也不能改变默认动画能力：
...
`skeletons.json` 从骨架视角聚合模型：`models` 只放可浏览的带 mesh 模型，动画 FBX 等无 mesh 的源骨架不会混入模型浏览列表。动画关系只作为显式诊断/历史迁移线索，不能成为默认 Library 候选。
...
全量 Library 不做海量模型 × 动画组合搜索。结构候选匹配只服务于显式小样本诊断；默认模型、贴图、材质、骨骼/skin 和索引主线不等待动画匹配。
...
如果只是改了索引结构、模型验证报告、素材库摘要或动画 out-of-scope 说明，不需要重新解包和导出模型，可以直接从已有 Library 导出目录重建索引：
...
它不会加载原始 Unity 游戏目录，也不会重新导出模型、贴图或动画；小样本通常能从数分钟缩短到数秒。限制是：导出时内存里才能拿到的 Unity 显式引用无法凭空恢复。需要刷新 Unity 关系图、CAB/PPtr 依赖、材质/贴图引用或新增素材时，仍然要重新跑 Library 导出。
...
### Endfield 默认 Library 导出阻断

Arknights Endfield 当前默认 `Library` 模型导出会被 CLI 直接拒绝，并在输出目录写 `export_refusal.json`。原因是该游戏的默认模型链路已知仍不能稳定产出完整可用素材库：近期烟测里 cutscene 包模型全部为 `warning`、`withTextures=0`，并且仍有材质/贴图 CAB 闭包与本机 VFS chunk 缺失问题。为避免长时间导出后才发现模型不可用，默认导出必须先停在入口。

以下诊断入口仍允许执行，用于补齐源索引、CAB 定位和材质贴图闭包：`--probe_source_input`、`--inspect_unity_files`、`--build_source_sqlite_index`、`--locate_endfield_cabs`、`--locate_endfield_missing_source_cabs`、`--build_endfield_cab_location_index`、`--locate_endfield_strings`、`--inspect_endfield_manifest_deps`。只有真实可复用模型样本重新证明 `model_validation=ok`、贴图/材质/skin/bbox 通过后，才应恢复默认 `Library` 导出。
...
`Animator` 模式主要用于排查 Animator Controller、动画收集和旧式内嵌动画，不作为默认素材库命令。默认 `--animation_package Skip` 会跳过 AnimationClip；如果确实要复现旧行为，需要显式传 `--animation_package Separate`、`Embedded` 或 `Both`，并把结果视为诊断/旧库迁移产物。
...
这会使用默认 `Library + ByLibrary + 完整可浏览模型素材库 + glTF + PNG + Skip animations`。模型和贴图可直接浏览，动画默认跳过并记录 out-of-scope 边界，不会把全局篮球动作重复塞进每个角色 glTF。
