...
重建 `library_index.db` 后，`assets` 表会把 `library_role`、`diagnostic_only`、`character_assembly_role`、`character_assembly_family`、`character_assembly_candidate_only`、`material_status` 和基础材质布尔值展开为显式列。统一浏览器或人工 SQL 可以直接筛选这些字段，不必每次从 `raw_json` 解析；但它们仍只继承 catalog 的诊断语义，不能绕过 `diagnosticOnly`、模型验证和动画门禁。
