...
每个 `transformNodeTableCandidates` 还会写 `usedBoneIndexCount`、`coveredUsedBoneIndexCount`、`missingUsedBoneIndexCount`、`coversAllUsedBoneIndices`、`exportedCoveredUsedBoneTransformCount` 和 `allCoveredUsedBoneTransformsExported`。这些字段只检查 `AvatarBoneWeights` 实际参与权重的非负 boneIndex 是否能按节点表顺序落到 `transformNodes.data`，以及对应 Transform JSON 是否已导出并能读到本地 TRS；它们比只看最大 boneIndex 更适合判断下一步该追哪张节点表，但仍不是 glTF skin 的放行条件。
