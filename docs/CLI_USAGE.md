...
- 默认 Library 会生成 `VFX/` 特效元数据目录：Unity `ParticleSystem`、`LineRenderer`、`TrailRenderer`、`VisualEffect`、GPU Particle/VFX 对象会进入 `vfx.json` / `VFX_REPORT.md`；mesh 型特效会在 `Models` 中标注 `resourceKind=VFX`。当前这是 metadata-first 管线，记录组件和来源，不代表 ParticleSystem/shader 动画已经完整烘焙为可播放标准格式。
