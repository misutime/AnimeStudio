...
非定向候选扫描在大源索引上采用 bounded scan：先从确定性 container/root 或 Renderer 关系里取有限数量的候选，再对这些候选补材质、贴图和容器统计。这样可以避免在 Naraka 这类上千万条 `assetBundle.containerPreload` 关系的源索引上，为了挑少量 smoke 样本先做全库聚合。它生成的是“可继续导出的线索”，不是全库最高分排行榜；需要指定角色、武器或场景时，应优先传 `--preview_model` / `--names` 做定向查询。
...
需要证明 verified preview 关系也能由当前 smoke 证据重新应用时，可以显式加 `-RefreshZhumuVerifiedAnimationPreview`。这个开关会改写 `-ZhumuScriptAnimationSampleRoot` 指向的样本库并重建它的 `library_index.db`，所以默认关闭；开启后，脚本会使用本轮生成的 `SourceModelAnimation_ZhumuSoul_ScriptAvatarDiagnostic\source_model_animation_candidates.json` 和 `zhumu_render_subject_occupancy.json` 调用 `--apply_verified_animation_preview`，并在 `smoke_summary.json.zhumuVerifiedAnimationPreviewRefresh` / `SMOKE_REPORT.md` 中记录刷新状态、merge report、render report、source report 和 import report。

`smoke_summary.json.firstUsableReadiness` 是当前一行 smoke 的机器验收入口。它按 `sourceInput`、`assetLibraryV1`、`representativeModels`、`textures`、`materials`、`skeleton`、`staticEnvironmentExtension`、`animationRelation`、`browserPreview` 汇总现有门禁。当前 Naraka 第一版应读作 `status=usableWithDegradedAnimation`、`firstUsable=true`、`productionReady=false`、`degradedAreas=["animationRelation"]`：模型、贴图、材质、骨骼和索引闭环已通过，动画是带边界的 verified preview 关系，还不是独立无 skin 动画库。若任何 area 变成 `blocked`，说明第一版 smoke 不可交付。

当前本机默认完整复验目录为 `D:\Assets\Naraka\Naraka_FirstUsableSmoke_FinalFull_Current`，执行命令为 `tools\Export-NarakaFirstUsableSmoke.ps1 -OutputRoot "D:\Assets\Naraka\Naraka_FirstUsableSmoke_FinalFull_Current" -RefreshZhumuVerifiedAnimationPreview`。通过标准是：`models=4`、`ok=4`、`withSkin=3`、`withTextures=4`、`textureLinkErrors=0`，AssetLibrary Browser 校验和 4 个代表模型缩略图渲染为 `ok`，`firstUsableReadiness.status=usableWithDegradedAnimation`、`firstUsable=true`、`blockedAreas=[]`、`degradedAreas=["animationRelation"]`；代表 Library 仍保持 `asset_library.json.capabilities.animations=false`、`animationSupport.defaultModelAnimationCandidateCount=0`、`modelAnimationRelations=0`。如果要回答“动画关系是否完全没找到”，优先查看 `smoke_summary.json.narakaAnimationRelationPolicy`、`sourceIndexScriptAnimationClipScripts`、`sourceIndexSimpleAnimationClipDomains`、`zhumuScriptAnimationProbe` 和 `animationRelationCoverage.sourceIndexAnimationRelationHealth`：当前生产 AnimatorController 链仍为 0，但 MonoBehaviour/SimpleAnimation 脚本字段里存在大批 AnimationClip PPtr 关系，其中 Zhumu 已经有一个单独的 `VerifiedAnimationPreview` 样本库通过门禁，`capabilities.animations=true`、`Animation=1`、`modelAnimationCandidates=1`、`relationAnimations=1`、`productionRelationTier=VerifiedPreviewRelation`、`relationPath=NarakaSimpleAnimationCustomRuntime`、`previewOnly=true`、`embeddedModelRequired=true`。也就是说，当前 Naraka 第一版动画不再把普通 AnimatorController 当成唯一生产入口；已验证脚本/PlayableGraph 风格关系可以作为生产可用的模型绑定预览关系，但还不是独立无 skin 动画库，也不是普通 AnimatorController 恢复。未经过 glTF TRS 覆盖、渲染运动和模型静态门禁的脚本字段仍保持 `blocked`。
...
根目录 `LIBRARY_README.md` 会从 `model_animations.compact.json` 写出“动画支持状态”：正式动画资产数量、默认模型-动画候选数量、模型动画门禁 ready/blocked/modelReady 计数和主要阻塞原因。如果当前库没有生产动画资产或默认候选，README 会明确说明动画仍处于诊断或待绑定阶段，不影响模型、骨骼、贴图和材质作为静态素材使用。
