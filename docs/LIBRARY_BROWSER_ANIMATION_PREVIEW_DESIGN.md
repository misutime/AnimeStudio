...
AnimationClip 也有同一类可追溯接入口：`UnityBakeProject/Assets/AnimeStudioBake/ImportedAnimationClip/*.anim` 和 `unityAnimationClips` 映射。它只在选中候选后决定 request 里的 `unityAssetPaths.animationClip`，不新增模型-动画候选；匹配 key 来自当前实际 bake 的 clip 名/文件名。对 Ambor 这类已通过 AnimatorController context 修正的候选，key 是 base body clip，例如 `Ani_Avatar_Girl_RunCycle`，这样 Unity helper 会直接加载原始/预导入的 Unity `AnimationClip` asset，并在结果里写出 `animationClipSource=unityAssetPaths.animationClip` 与 `isHumanMotion`。没有命中时仍可回退导出的 `.anim` sidecar，但 Humanoid/Muscle 生产路径必须通过 Unity `isHumanMotion` 检查，否则失败而不是生成误导 glTF。

CLI 会在新导出或 `--build_sqlite_index` 后自动尝试恢复 ImportedAnimationClip，前提是素材库或全局设置里已经配置 Unity bake project。恢复器只读取 `library_index.db` 里的 `relation_source=explicit` Humanoid/Muscle 候选；当候选是 AnimatorController 辅助 clip 时，恢复器使用 `animatorControllerContext.baseLayerClip` 作为实际身体动画，不按动画名、目录或骨骼数量猜测。恢复结果写入 `Assets/AnimeStudioBake/ImportedAnimationClip/` 和素材库本地 `unityAnimationClips` 映射，Browser 后续 bake 会自动引用。
...
- `可播放`：非 Humanoid/Unity bake 预览需要 glTF 和 `preview_validation.json` 通过当前最小检查；Humanoid/Unity bake 预览必须同时具备 baked glTF、`unity_bake_apply_report.json`、`frameVaryingTracks > 0`、`avatarTrust.TrustedProductionBake=true`，且显式 Avatar asset request 必须证明 `avatarTrust.source=imported_unity_avatar_asset`。只有文件存在但缺这些证明时，只能显示“已烘焙但需重建”“静态姿态”“需人工验收”或失败/复查状态。
...
- `需 AnimatorController 上下文`：候选关系是 Unity 显式引用，但当前单个 `AnimationClip` 缺少完整身体驱动，或 Unity 导入后不是 `isHumanMotion`。这类结果不能硬生成 glTF；需要恢复状态机、layer、blend tree 或组合 clip 后再 bake。Ambor 这类角色专属 bow clip 已验证会出现这种情况：原 clip 本身是 controller 辅助片段，必须先按 Unity controller 上下文换成同一状态的完整身体 clip，Browser/CLI 才允许进入生产 bake。
- `拒绝预览` / `拒绝烘焙`：候选不是 Unity 显式关系，Browser 不会把结构匹配、骨骼兼容、名称匹配或人工拼接写成默认可播放缓存。

模型详情里的默认动画列表也只显示显式关系。结构兼容、骨骼路径或人工定向匹配结果只能作为诊断文本出现；如果旧索引或诊断入口带回非显式候选，列表会显示“诊断隐藏”提示，而不会让它们进入双击预览、批量烘焙或可播放缓存。
旧 `confidence=explicit_unity_reference` 只能说明候选质量，不能单独把候选升级为显式关系；Browser 和 CLI 默认入口都要求 `relationSource=explicit`。如果旧库缺少这个字段，应重建 SQLite/动画索引，而不是继续用 confidence 兜底生成动画。
...
打开素材库时，Browser 会读取根目录的 `animation_bake_cache_summary.json`，在状态栏和模型详情中显示全局 Unity bake oracle 覆盖、普通 prefab/HumanDescription 路径候选数、ImportedAvatar 路径候选数、可信 baked glTF 数、静态姿态和不可信 baked 统计。Browser 必须同时显示这个摘要相对 `sqlite_index_summary.json` 的 fresh/stale/missing 状态；如果 bake cache 早于当前 SQLite 索引，只能提示为过期，不能让用户误以为覆盖率已经对应最新关系库。这个摘要只来自 CLI 批量烘焙或 `--preview_validation_limit 0` 刷新，不会在 Browser 加载时重新扫描几百万候选，避免打开大库时卡住。
Browser 内触发单个或批量 Unity bake 后，会重新读取已有 `animation_bake_cache_summary.json`，让状态栏和详情区尽快看到 CLI 写回的 cache 变化；但 Browser 不会自动跑全量 `--preview_validation_limit 0` 统计，避免一次 UI 操作卡住原神这类大库。模型详情也会读取最新的 Browser 批量报告，若没有 Browser 报告则读取 CLI 默认输出 `UnityBakedAnimationPreviews/unity_bake_batch_report.json`，显示最近一次状态、请求数、是否运行 Unity、成功、失败、已烘焙和缺 Avatar oracle 跳过数量；这只是操作审计，不写入全局候选关系，也不改变缓存覆盖率。
...
当 GateOnly 报告显示 `candidateTableSchema.status != ok`，但 `totals.nonExplicitCandidates == 0` 时，说明现有候选数据是干净的、只是旧 `library_index.db` schema 还没有硬约束。Browser 顶部“重建动画索引”按钮会运行 `--build_sqlite_index`，并默认带上 `--require_fresh_source_animation_relations --skip_sqlite_file_index --skip_sqlite_sidecar_scan --skip_sqlite_json_documents`，只刷新结构化索引、显式动画候选和候选表 schema，不重新导出模型、贴图或动画。这个按钮会优先复用 `sqlite_index_summary.json` 记录的 `sourceIndex`；如果该路径不存在，才回退到素材库根目录 `unity_source_index.db`。源索引缺少当前工具要求的 Animator/Animation/OverrideController 精确关系时，应失败并要求先重建源索引，不能退回骨骼数量、名称或路径猜测。VRising 这类旧库如果先在旁路目录生成了 fresh 源索引，应先用 CLI `--build_sqlite_index <LibraryRoot> --source_index <FreshUnitySourceIndex>` 显式刷新一次正式 `library_index.db`，让 summary 记录实际源索引；之后 Browser 按钮会继续复用这份 fresh 源索引。
