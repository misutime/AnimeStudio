...
Browser 也会读取最新的 `AnimationRelationDiagnostics*/deterministic_animation_coverage.json` 或根目录同名报告，在状态栏和模型详情中显示“动画关系门禁”。这个结果只来自 `Measure-DeterministicAnimationCoverage.ps1 -GateOnly/-FailOnWarning` 等离线命令；Browser 打开素材库时不会主动执行 SQLite 全库诊断，避免大库 UI 卡顿。
