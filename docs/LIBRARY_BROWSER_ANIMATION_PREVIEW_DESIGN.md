...
旧 `model_animations.json` 入口也必须遵守同一条规则：`--generate_preview_gltf`、`--pack_model_animations` 和 `--generate_unity_bake_request` 只能处理显式 Unity 关系候选。嵌入 sidecar、结构匹配、名称匹配或手工选择可以保留为诊断数据，但不能进入默认可播放 glTF、动画包或生产 Unity bake 请求。
