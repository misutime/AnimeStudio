...
- `需 AnimatorController 上下文`：候选关系是 Unity 显式引用，但当前单个 `AnimationClip` 缺少完整身体驱动，或 Unity 导入后不是 `isHumanMotion`。这类结果不能硬生成 glTF；需要恢复状态机、layer、blend tree 或组合 clip 后再 bake。
