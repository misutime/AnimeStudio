...
- 静态环境/建筑/道具 Mesh 是默认 `Library` 的一等资源，不是次要补丁；但不能默认导出没有任何静态素材语义的匿名 Mesh、碰撞、NavMesh、Occlusion、Dummy、Socket、Joint、Bone 或 obsolete/deprecated 资源。`Decal`、`Shadow`、`SFX/FX/VFX` 等可能是有效可见素材时，优先分类、标注或报告，不要仅凭名字静默丢弃。裸 Mesh 没有 Renderer 材质绑定时只能记录同容器/同源材质候选或标记 `missingRendererMaterial`，不能伪造材质关系。匿名静态 Mesh 要用来源包名 + PathID 生成稳定文件名，不能让 `Mesh#123` 这类临时名成为素材库入口。
