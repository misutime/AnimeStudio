...
- VFX 是默认 `Library` 的一等资源域。粒子、拖尾、线渲染、VisualEffect、GPU Particle 和 mesh/shader 型特效应进入 `VFX/` 元数据索引或 `Models` 中的 `resourceKind=VFX`，不要因为名称包含 `fx` / `vfx` / `sfx` 就静默过滤。当前策略是 metadata + preview hints：先记录 Unity 组件、来源、mesh 预览和限制，并尽可能从 TypeTree 轻量解析 `ParticleSystem` / `ParticleSystemRenderer` 的 Main、Emission、Shape、Size、Color、Trail、renderMode 等预览参数，同时通过源索引记录 VFX/Renderer/Material 到 Texture 的轻量 PPtr 关系；没有完整还原 shader 动画、Texture Sheet 图集、VFX Graph 或动画事件时，必须在 `vfx.json` / `VFX_REPORT.md` 明确标注，不能伪装成已完整可播放。LibraryBrowser 的 VFX 缩略图/预览要优先使用这些 `previewHints`，再结合名称、分类、组件、材质、贴图和 Mesh 引用模拟真实效果类型，至少区分 trail、shockwave、aura、smoke、projectile、beam、distortion、mesh particle、ground plane、stretch billboard、textured billboard 等形态；未烘焙 runtime 数据前必须标注为 approximate，不能所有 VFX 共用一个小圆点占位模板。VFX 不能影响模型、贴图、骨骼、动画等核心导出链路；如果两者冲突，核心资产稳定性优先。
