...
- SQLite 分两类：`library_index.db` 面向已导出的素材库目录，`unity_source_index.db` 面向完整 Unity 源目录。源索引必须记录 source file、SerializedFile、Object、external CAB/PPtr、核心 Unity 关系和 AnimationClip binding；全量 Library 导出必须传 `--source_index` 使用 SQLite 源索引作为依赖底座。旧 CAB map / AssetMap 只作为显式 `--map_op` 调试或兼容旧流程，不能作为精品全量导出的默认机制。
